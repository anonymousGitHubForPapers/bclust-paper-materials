%% load seeds data
clear;
raw = readmatrix("raw_algerian_sidi_bel_abbes.txt"); % last column is label, the others are the data (https://archive.ics.uci.edu/dataset/547/algerian+forest+fires+dataset)
label = raw(:,end);
data = raw(:,1:(end-1));
...
for i = 1:N
    data(:,i) = (data(:,i)-mean(data(:,i)))/std(data(:,i) + 0.00001);
end
%% init rng
rng(42);
%% do benchmark
